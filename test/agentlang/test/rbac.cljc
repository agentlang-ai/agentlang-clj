  ...
  (call-with-rbac
   (fn []
     (let [with-u1 (partial with-user "u1@pap.com")
       ...
       (is (not (seq (fc with-u2 4 1 100))))
       (is (ip? (assign-inst-priv "u1@pap.com" "u2@pap.com" [:read] (as-path :Pap/A 1))))
       (is (cn/same-instance? a1 (first (fa with-u2 1))))
       (is (cn/same-instance? b1 (first (fb with-u2 100 1))))
       (is (cn/same-instance? c1 (first (fc with-u2 4 1 100))))))))
