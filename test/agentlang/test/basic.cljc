...
#? (:clj
    ...
    :cljs
    (def store (store/open-default-store {:type :mem})))
...
(defn- valid-opcode-with-query? [opcode farg]
  (is (opc/query-instances-opcode? opcode))
  (let [arg (opc/arg opcode)]
    (is (= farg (first arg)))))

(def ^:private load-instance? (partial valid-opcode? opc/load-instance-opcode?))
     ...
     (let [[_ c] (pattern-compiler)
           p1 {:CompileTest/E1
               {:X 100
                :Y 200}}
           opcs (c p1)]
       (is (valid-opcode? opc/new-instance-opcode?
                          (first opcs) [:CompileTest :E1]))
       #_(is (valid-opcode? opc/set-literal-attribute?
                            (second opcs) [:X 100]))
       #_(is (valid-opcode? opc/set-literal-attribute?
                            (nth opcs 2) [:Y 200]))
       (is (valid-opcode? opc/intern-instance-opcode?
                          (nth opcs 3)
                          (fn [[r a _ v u]]
